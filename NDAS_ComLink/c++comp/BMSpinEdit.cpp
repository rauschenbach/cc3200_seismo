...
//---------------------------------------------------------------------------
void __fastcall TBMSpinEdit::TAGauge::SetGuageBeginColor ( TColor Value )
{
  if ( FGuageBeginColor != Value )
    {
    FGuageBeginColor = Value;
    Invalidate();
    }

}
//---------------------------------------------------------------------------
void __fastcall TBMSpinEdit::TAGauge::SetGuageEndColor ( TColor Value )
{
  if ( FGuageEndColor != Value )
    {
    FGuageEndColor = Value;
    Invalidate();
    }
...
//  int Correction = ( FEdit->Value - FEdit->MinValue ) * 256 / Diff + 1;//SolveForX(PercentDone, W);
  ...
  TColor BeginColor = (TColor)ColorToRGB( FGuageBeginColor );
  TColor EndColor = (TColor)ColorToRGB( FGuageEndColor );
...
void __fastcall TBMSpinEdit::SetButtonHandle()
{
  ...
  if ( FTrackBarOrientation == trVertical )
    {
    FSpeedButton->Glyph->Handle = LoadBitmap((void*) HInstance, "DropLeft");
    ...
    {
    FSpeedButton->Glyph->Handle = LoadBitmap((void*) HInstance, "DropDown");
...
void __fastcall TBMSpinEdit::SetBounds(int ALeft, int ATop, int AWidth, int AHeight)
{
  if ( FPanel )
    ...
    FPanel->SetBounds ( AWidth - 4 - FUpDown->Width - FSpW - 3, 0, FUpDown->Width + FSpeedButton->Width + 3, AHeight - 3 - GuageHeight - 1 );
...
//---------------------------------------------------------------------------
void __fastcall TBMSpinEdit::SetGuageBeginColor( TColor Value )
{
  ProgressBar->GuageBeginColor = Value;
}
//---------------------------------------------------------------------------
TColor __fastcall TBMSpinEdit::GetGuageBeginColor()
{
  return ProgressBar->GuageBeginColor;
}
//---------------------------------------------------------------------------
void __fastcall TBMSpinEdit::SetGuageEndColor( TColor Value )
{
  ProgressBar->GuageEndColor = Value;
}
//---------------------------------------------------------------------------
TColor __fastcall TBMSpinEdit::GetGuageEndColor()
{
  return ProgressBar->GuageEndColor;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TBMSpinEdit::SetGuageHeight ( int Value )
{
  if( FGuageHeight != Value )
    {
    FGuageHeight = Value;
    SetBounds( Left, Top, Width, Height ); 
    }
