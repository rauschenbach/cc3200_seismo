...
static ADS131_DATA_h adc_pack;	/* Пакет с данными на отправление  - статический буфер  - 272 байта */
...
/* 
 * Синк объект для записи на SD - будет ожидать сообщение
 * которое приходит из прерывания.  семафор или что то еще
 */
...
/*vvvvv		adc_irq_unregister(); */
	...
	/* Настраиваем регистры для МАКСИМАЛЬНОГО!!! каналов */
...
void ADS131_start(void)
{
    ...
    /* Все эти действия выполняем 1 раз! */
    OsiReturnVal_e ret;
...
static void cmd_mode_handler(u32 * data, int num)
{
	...
	pack->t0 = ms * 1000000;	/* Секунда + миллисекунды первого пакета. время UNIX_TIME_t */
	pack->adc_freq = adc_pars_struct.sps_code;	/* частота дискретизации */
    }
...
bool ADS131_write_parameters(ADCP_h * v)
{
	...
	par.sps = (ADS131_FreqEn) adcp.adc_param.sample_rate;	/* частота  */
	...
	par.file_len = 1;	/* Длина файла записи в часах */
