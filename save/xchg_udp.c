...
static XCHG_DATA_STRUCT_t *pRxBuf = (XCHG_DATA_STRUCT_t *) XCHG_RX_MEM_ADDR;	/* Указатель на нашу структуру приема! */
static XCHG_DATA_STRUCT_t *pTxBuf = (XCHG_DATA_STRUCT_t *) XCHG_TX_MEM_ADDR;	/* Указатель на нашу структуру передачи! */
static XCHG_COUNT_STRUCT_t *pCount = (XCHG_COUNT_STRUCT_t *) XCHG_COUNT_MEM_ADDR;	/* Счетчики обмена */
#else
