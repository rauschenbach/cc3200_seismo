...
// Macros
//*****************************************************************************
#define  SD_CARD_CLOCK_FREQ    16000000UL
...
//
//*****************************************************************************
DRESULT disk_read(BYTE bDrive, BYTE * pBuffer, DWORD ulSectorNumber, UINT bSectorCount)
{
	DRESULT Res;
...
//
//*****************************************************************************
DRESULT disk_write(BYTE bDrive, const BYTE * pBuffer, DWORD ulSectorNumber, UINT bSectorCount)
{
	DRESULT Res;
	unsigned long ulSize;
...
DWORD get_fattime(void)
{
 ...
 /* месяцы в tm_time с 0 по 11, годы считаются с 1900 
  * Я знаю приоритеты операций! */
   return ((DWORD) (tim.tm_year - 80) << 25)
	    | ((DWORD) tim.tm_mon + 1 << 21)
	    | ((DWORD) tim.tm_mday << 16)
	    | ((DWORD) tim.tm_hour << 11)
	    | ((DWORD) tim.tm_min << 5)
	    | ((DWORD) tim.tm_sec / 2 >> 1);
}
...
void sd_card_mux_config(void)
{
	...
	/* Enable Pull up on SDHost_D0 - white */
	MAP_PinConfigSet(SD_CARD_DATA0_PIN, PIN_STRENGTH_6MA, PIN_TYPE_STD_PU);

	/* Configure SDHost_D0 for SDHOST0  */
	MAP_PinTypeSDHost(SD_CARD_DATA0_PIN, SD_CARD_DATA0_MODE);

	/* Enable Pull up on CMD - green */
	MAP_PinConfigSet(SD_CARD_CMD_PIN, PIN_STRENGTH_6MA, PIN_TYPE_STD_PU);

	/* Configure PIN_08 for SDHOST0 SDHost_CMD */
	MAP_PinTypeSDHost(SD_CARD_CMD_PIN, SD_CARD_CMD_MODE);

	/* Configure SDHost_CLK for SDHOST0  */
	MAP_PinTypeSDHost(SD_CARD_CLK_PIN, SD_CARD_CLK_MODE);

	/* Set the SD card clock as output pin - gr/ye */
	MAP_PinDirModeSet(SD_CARD_CLK_PIN, PIN_DIR_MODE_OUT);
	...
	/* Configure card clock - потом частоту поменять на более высокую !!! */
	MAP_SDHostSetExpClk(SDHOST_BASE, MAP_PRCMPeripheralClockGet(PRCM_SDHOST), SD_CARD_CLOCK_FREQ);
}
